...
def creation_date(path_to_file):
    ...
    if re.match(r'^20[0-9]{6}\_[0-9]{6}', filename):
    ...
    try:
        output=subprocess.run(['exiftool', path_to_file], stdout=subprocess.PIPE).stdout.decode('utf-8')
        exif = dict()
        for line in output.splitlines():
            attr = re.split(r'\s*:\s+', line, maxsplit=1)
            if len(attr) == 2:
                exif[attr[0]] = attr[1]
        for p in ('Create Date', 'Date Created', 'Date/Time Original'):
            if p in exif:
                try:
